...
numClusters = 2;
[height, width] = size(medianFilteredImage);
pixelData = reshape(medianFilteredImage, height*width, 1);
[clusterIndices, ~] = kmeans(pixelData, numClusters, 'MaxIter', 1000);

% Create a binary image from the cluster indices
kmeansSegmentedImage = reshape(clusterIndices, height, width);
kmeansSegmentedImage = kmeansSegmentedImage > 1;

% Show the K-means segmented image
figure;
imshow(kmeansSegmentedImage);

% Calculate the true positives, false positives, false negatives, and true negatives for the K-means segmented image
TP_kmeans = sum(sum(kmeansSegmentedImage & groundTruthImage));
FP_kmeans = sum(sum(kmeansSegmentedImage & ~groundTruthImage));
FN_kmeans = sum(sum(~kmeansSegmentedImage & groundTruthImage));
TN_kmeans = sum(sum(~kmeansSegmentedImage & ~groundTruthImage));

% Calculate the accuracy, precision, recall, and F for the K-means segmented image
accuracy_kmeans = (TP_kmeans+TN_kmeans)/(TP_kmeans+FP_kmeans+FN_kmeans+TN_kmeans);
precision_kmeans = TP_kmeans/(TP_kmeans+FP_kmeans);
recall_kmeans = TP_kmeans/(TP_kmeans+FN_kmeans);
F_kmeans = 2*(precision_kmeans*recall_kmeans)/(precision_kmeans+recall_kmeans);
...
fprintf('Evaluation metrics for K-means clustering-based segmentation:\n');
fprintf('Accuracy: %f\n', accuracy_kmeans);
fprintf('Precision: %f\n', precision_kmeans);
fprintf('Recall: %f\n', recall_kmeans);
fprintf('F-measure: %f\n', F_kmeans);